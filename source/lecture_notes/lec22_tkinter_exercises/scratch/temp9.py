...
class MyApp(object):
    def __init__(self, parent):
        ...
        self.rep_var = StringVar()
        self.rep_var.set( "#circles = {:d}".format(self.repetitions))
        self.circle_label = Label(self.draw_frame, textvariable=self.rep_var)
        self.circle_label.pack(side=TOP)

        self.main_canvas = Canvas(self.draw_frame, \
                                  height=self.canvas_dimension,
                                  width=self.canvas_dimension)
        self.main_canvas.pack(side=BOTTOM)
