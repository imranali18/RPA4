...
class Restaurant(object):
    def __init__(self, name, lat, lon, address, url, category, scores):
        ...
        """
        self.name = name
        self.loc = Point2d(float(lat), float(lon))
    ...
    def __str__(self):
        ...
        Rather than outputting the whole list of reviews, the average review
        is output.
        """
        s =  '      Name: ' + self.name + '\n'
    ...
    def is_in_city(self, city_name):
        ...
        """
        city_from_address = self.address[-1].split(',')[0].strip()
        return city_name.lower() == city_from_address.lower()
